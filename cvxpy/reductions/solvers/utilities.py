...
def extract_dual_vars_from_solver(solution_z, zero_dim, parse_func,
                                   eq_constr, ineq_constr) -> Dict[Any, Any]:
    """Extract dual variables from solver solution for equality and inequality constraints.

    This is a common pattern used by conic solvers to extract dual variables
    from the solution vector, splitting between equality and inequality constraints.

    Parameters
    ----------
    solution_z : array_like
        The dual solution vector from the solver.
    zero_dim : int
        The dimension of the zero cone (number of equality constraints).
    parse_func : function
        A function that extracts a dual value from the result vector
        for a particular constraint (e.g., solver.extract_dual_value).
    eq_constr : list
    ...
    -------
    Dict
        Merged dictionary of dual variables for all constraints.
    """
    eq_dual_vars = get_dual_values(
        solution_z[:zero_dim],
        parse_func,
        eq_constr
    )
    ineq_dual_vars = get_dual_values(
        solution_z[zero_dim:],
        parse_func,
        ineq_constr
    )
    return eq_dual_vars | ineq_dual_vars
